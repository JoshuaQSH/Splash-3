...
/*   pthread_mutex_t (section_lock)[MAX_PROCESSORS];*/  /* key locks */
   struct { pthread_mutex_t bar_mutex; pthread_cond_t bar_cond; unsigned bar_teller; } barrier_rank;                   /* for ranking process */
   struct { pthread_mutex_t bar_mutex; pthread_cond_t bar_cond; unsigned bar_teller; } barrier_key;                    /* for key sorting process */
   double *ranktime;
   double *sorttime;
   double *totaltime;
   long final;
...
/*   { int i; for(i = 0; i < (MAX_PROCESSORS); i++) pthread_mutex_init(&((global->section_lock)[i]), NULL); }*/
   {
	pthread_mutex_init(&((global->barrier_rank).bar_mutex), NULL);
	pthread_cond_init(&((global->barrier_rank).bar_cond), NULL);
	(global->barrier_rank).bar_teller=0;
}
   {
	pthread_mutex_init(&((global->barrier_key).bar_mutex), NULL);
	pthread_cond_init(&((global->barrier_key).bar_cond), NULL);
	(global->barrier_key).bar_teller=0;
}
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute the key,
...

   {
	...
	for (i = 0; i < (number_of_processors) - 1; i++) {
		Error = pthread_create(&__tid__[__threads__++], NULL, (void * (*)(void *))(slave_sort), NULL);
		if (Error != 0) {
...
/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
   ...
   {
pthread_mutex_lock(&((global->barrier_key).bar_mutex));
(global->barrier_key).bar_teller++;
if ((global->barrier_key).bar_teller == (number_of_processors)) {
	(global->barrier_key).bar_teller = 0;
	pthread_cond_broadcast(&((global->barrier_key).bar_cond));
} else {
	pthread_cond_wait(&((global->barrier_key).bar_cond), &((global->barrier_key).bar_mutex));
}
pthread_mutex_unlock(&((global->barrier_key).bar_mutex));}
...
/* generate histograms based on one digit */
     ...
     {
pthread_mutex_lock(&((global->barrier_rank).bar_mutex));
(global->barrier_rank).bar_teller++;
if ((global->barrier_rank).bar_teller == (number_of_processors)) {
	(global->barrier_rank).bar_teller = 0;
	pthread_cond_broadcast(&((global->barrier_rank).bar_cond));
} else {
	pthread_cond_wait(&((global->barrier_rank).bar_cond), &((global->barrier_rank).bar_mutex));
}
pthread_mutex_unlock(&((global->barrier_rank).bar_mutex));}
